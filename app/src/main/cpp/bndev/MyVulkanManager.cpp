...
/// Sample6_6
ShaderQueueSuit_Moon *MyVulkanManager::sqsSTL;
ShaderQueueSuit_Earth *MyVulkanManager::sqsDTL;
ColorObject *MyVulkanManager::skyForDrawBig;
ColorObject *MyVulkanManager::skyForDrawSmall;
DrawableObjectCommon *MyVulkanManager::planetForDraw;
...
void MyVulkanManager::create_vulkan_devices() {
  ...
  /// Sample4_7、Sample6_11 *************************************** start
  VkPhysicalDeviceFeatures pdf;                                             // 构建物理设备属性实例
  vk::vkGetPhysicalDeviceFeatures(gpus[0], &pdf);                           // 获取物理设备属性
  if (pdf.wideLines == VK_TRUE) {                                           // 判断是否支持wideLines特性
    LOGI("support wideLines feature!");
  } else {
    LOGE("don't support wideLines feature!");
  }
  assert(pdf.samplerAnisotropy == VK_TRUE);                                 // Sample6_11-检查是否支持各向异性过滤
  /// Sample4_7、Sample6_11 ***************************************** end
...
//  objForDraw = LoadUtil::loadFromFile("model/ch_t.obj", device, memoryroperties);
  ...
  /// Sample7_6 ************************************************** start
  objForDraw = LoadUtil::loadFromFile("model/ch_no_t.obj", device, memoryroperties);
  /// Sample7_6 **************************************************** end
}
