...
void ShaderQueueSuit_Earth::create_uniform_buffer(VkDevice &device, VkPhysicalDeviceMemoryProperties &memoryroperties) {
...
void ShaderQueueSuit_Earth::destroy_uniform_buffer(VkDevice &device) {
  vk::vkDestroyBuffer(device, uniformBuf, NULL);
  vk::vkFreeMemory(device, memUniformBuf, NULL);
}

void ShaderQueueSuit_Earth::create_pipeline_layout(VkDevice &device) {
...
void ShaderQueueSuit_Earth::destroy_pipeline_layout(VkDevice &device) {
...
void ShaderQueueSuit_Earth::init_descriptor_set(VkDevice &device) {
  VkDescriptorPoolSize type_count[3];
  type_count[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
  type_count[0].descriptorCount = TextureManager::texNamesPair.size() / 2;
  type_count[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  type_count[1].descriptorCount = TextureManager::texNamesPair.size() / 2;
  type_count[2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
  type_count[2].descriptorCount = TextureManager::texNamesPair.size() / 2;
  VkDescriptorPoolCreateInfo descriptor_pool = {};
  descriptor_pool.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
  descriptor_pool.pNext = NULL;
  descriptor_pool.maxSets = TextureManager::texNamesPair.size() / 2;
  ...
  for (int i = 0; i < TextureManager::texNamesPair.size() / 2; i++) {
  ...
  alloc_info[0].descriptorSetCount = TextureManager::texNamesPair.size() / 2;
  alloc_info[0].pSetLayouts = layouts.data();
  descSet.resize(TextureManager::texNamesPair.size() / 2);
...
void ShaderQueueSuit_Earth::create_shader(VkDevice &device) {
  std::string vertStr = FileUtil::loadAssetStr("shader/sample6_6-moonearth.vert");
  std::string fragStr = FileUtil::loadAssetStr("shader/sample6_6-earth.frag");
...
void ShaderQueueSuit_Earth::create_pipe_line(VkDevice &device, VkRenderPass &renderPass) {
...
void ShaderQueueSuit_Earth::destroy_pipe_line(VkDevice &device) {
  vk::vkDestroyPipeline(device, pipeline, NULL);
  vk::vkDestroyPipelineCache(device, pipelineCache, NULL);
}

ShaderQueueSuit_Earth::ShaderQueueSuit_Earth(VkDevice *deviceIn,
...
ShaderQueueSuit_Earth::~ShaderQueueSuit_Earth() {
